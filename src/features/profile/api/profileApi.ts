...
export const fetchRatings = async (
	...
	const { data, error } = await supabase
		.from('ratings')
		.select('id, item_type, spotify_id, name, image_url, artist_name, rating, updated_at, release_date')
		.eq('profile_id', profileId)
...
export const fetchDiary = async (
	...
	const { data, error } = await supabase
		.from('diary')
		.select('id, action, item_type, spotify_id, name, image_url, artist_name, rating, review_text, created_at')
		.eq('profile_id', profileId)
...
export const fetchFollowers = async (profileId: string): Promise<FollowRow[]> => {
	const { data, error } = await supabase
		.from('follows')
		.select('*')
...
export const fetchFollowing = async (profileId: string): Promise<FollowRow[]> => {
	const { data, error } = await supabase
		.from('follows')
		.select('*')
...
export const createPlaylist = async (
	...
	const { data, error } = await supabase
		.from('playlists')
		.insert({ profile_id: profileId, title, description: description ?? null })
		.select()
		.single();
	if (error) throw error;
...
export const fetchPlaylists = async (profileId: string): Promise<PlaylistRow[]> => {
	const { data, error } = await supabase
		.from('playlists')
		.select('*')
		.eq('profile_id', profileId)
...
export const fetchPlaylistItems = async (playlistId: string): Promise<PlaylistItemRow[]> => {
	const { data, error } = await supabase
		.from('playlist_items')
		.select('*')
