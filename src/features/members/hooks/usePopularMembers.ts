...
import { featuredMembersData, popularThisWeekData } from '@/features/members/data/membersData'
...
export const usePopularMembers = ({ since }: UsePopularMembersOptions): UsePopularMembersResult => {
        ...
        fetchMembersGrid(since, 6)
            .then((rows) => {
                ...
                if (rows.length >= 3) {
                    ...
                    setData(since === null ? featuredMembersData : popularThisWeekData)
                }
            })
            .catch(() => {
                if (!cancelled) {
